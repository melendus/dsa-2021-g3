...
void R(int id,ListT *road)  //R este de creare a unui nod/truck si plasarea lui intr-o lista
{
    NodeT *truck;
...
void E(int id, ListT *garage,ListT *road,char *pt1) //mutarea unui truck de pe road in garaj, adica din lista in stack
{
    //FILE *argv=fopen(pt1,"a");
    NodeT *truck;
    ...
    {
        FILE *argv=fopen(pt1,"a");
        fprintf(argv,"error: %d not on road\n",id);
        fclose(argv);
        return;
    }
    else insertAtFront(garage,truck);
}

void X(int id, ListT *road, ListT *garage,char *pt1)
{
    //FILE *argv=fopen(pt1,"a");
    NodeT *truck;
